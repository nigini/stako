...
function insertDesign(banner) {
  // Add tracking of the page's original elements
  ...
  for(let element of users) {
    var tagChildren = element.children;
    var avatar = false;
    ...
    if(avatar) {
      ...
      //element.classList.remove("user-hover");
  ...
  if(type === value[0]) {
    ...
    for(let element of tags) {
      ...
      getTimeIn(tag, "crew_tab");
      getTimeOut(tag, "crew_tab");
      trackClick(tag, "crew_tab");
    }
...
function getTimeIn(element, origin="original") {
...
*/
function getTimeOut(element, origin="original") {
  element.addEventListener('mouseleave', function (e) {
    ...
    if(timeIn && totalTime >= DELAY) {
      ...
      if(element.classList.contains("post-tag")) {
        ...
        chrome.runtime.sendMessage({extensiondId: "background.js", type: "stackoverflow:mouse", url: window.location.href, time: totalTime, ele: tag_link, origin: origin}, function(response){
          //console.log(response.testType + " " + response.testURL + " " + response.testTime + " " + response.origin);
        });
...
function trackClick(element, origin="original") {
  //Tracks whether one of the elements of interest has been clicked on.
  if(element.classList.contains("post-tag")){
    element.addEventListener('click', function (e) {
      var tag_link = "https://stackoverflow.com/" + element.href;
      chrome.runtime.sendMessage({extensiondId: "background.js", type: "stackoverflow:click", url: window.location.href, ele: tag_link, origin: origin}, function(response) {
        //console.log(response.testType + " " + response.testURL + " " + response.origin);
      });
    });
  } else if(element.classList.contains("user-info")){
    var linkElement = element.querySelector(".user-gravatar32 > a");
